...
## PLEASE ENSURE THE STANFORD NER MODEL IS UNZIPPED AND IN THE OUTERMOST DIRECTORY
...
def get_persons_and_positions(results, text):
    """
    Getting each entity labelled as PERSON and it's position within the text
    """
...
def get_fullnames(persons_and_positions):
    """
    Combining names if they follow each other (using person position) to get full names
    """
...
def get_person_counts(fullnames_and_positions):
    """
    Gettng total counts that each name is mentioned and combing locations for next step
    """
    person_counts = {}
...
def get_section_of_text(preprocessed_text,start, end):
    """
    Returning text 100 words before and after a given start and end of a word
    """
...
def get_response(url:str):
    """
    Main function to return required output
    """
    # get text from page
